            ...
            <div class="md:col-span-1">
                        ...
                        <?php else: ?>
                            <div class="w-32 h-32 rounded-full bg-gradient-to-r from-purple-600 to-pink-600 flex items-center justify-center mb-4">
                                <span class="text-white text-4xl font-medium">
                                    <?= isset($userData['display_name']) && !empty($userData['display_name']) ? 
                                        substr($userData['display_name'], 0, 1) : 
                                        substr($userData['username'], 0, 1) ?>
                                </span>
                            </div>
                        <?php endif; ?>
                        <h2 class="text-xl font-bold text-white"><?= htmlspecialchars($userData['display_name'] ?? $userData['username']) ?></h2>
                        <p class="text-gray-400"><?= htmlspecialchars($userData['email']) ?></p>
                        <p class="text-gray-400 mt-1">Member since <?= date('F Y', strtotime($userData['created_at'])) ?></p>
                    </div>
                ...
                <div class="bg-gray-800 rounded-lg p-6">
                    ...
                    <div class="flex items-center mb-2">
                        ...
                        <span class="<?= $subscription['status'] === 'premium' ? 'text-yellow-400' : 'text-white' ?>">
                            <?= $subscription['plan'] ?>
                        </span>
                    </div>
                    
                    <?php if ($subscription['expires']): ?>
                    <div class="flex items-center mb-4">
                        <span class="text-gray-300 mr-2">Expires:</span>
                        <span class="<?= $subscription['status'] === 'expired' ? 'text-red-400' : 'text-white' ?>">
                            <?= date('F j, Y', strtotime($subscription['expires'])) ?>
                        </span>
                    </div>
                    <?php endif; ?>
                    
                    <a href="subscription.php" class="w-full block text-center bg-purple-600 hover:bg-purple-700 text-white font-medium py-2 px-4 rounded-lg transition-colors">
                        <?= $subscription['status'] === 'premium' ? 'Manage Subscription' : 'Upgrade to Premium' ?>
                    </a>
                </div>
            </div>
